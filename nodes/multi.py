...
class ProduceOutput:
    def __init__(self, node: 'BaseMultiWatcherNode', filename, mode):
        self.node = node
...
class BaseMultiWatcherNode(threading.Thread, ABC):
    def __init__(
        self, 
        name: str, 
        input_directories: List[Dict[str, int]],
        ...
        Args:
            ...
            input_directories (List[Dict]): List of paths with desired artifacts_per_run.
            {
                'data_store1_output': {
                    'artifacts_per_run': 1, 'hash_chunk_size': 1_048_576
                },
                'data_store2_output': {
                    'artifacts_per_run': 1, 'hash_chunk_size': 1_048_576
                }
            }
            output_paths (List[str]): List of output paths for this node.
        """
        self.input_directories: Dict[str, Dict[str, int]] = input_directories

        self.input_paths = [input_directory_dict['directory'] for input_directory_dict in input_directories]
        ...
        self.successors = []
        self.predecessors = input_directories
        ...
        self.node_id = f"{name}|{'|'.join(self.input_paths)}|{'|'.join(self.output_paths)}"
    ...
    def start_monitoring(self) -> None:
        ...
        def _monitor_thread_func():
            ...
            while self.exit_event.is_set() is False:
                for input_path, config in self.input_directories.items():
                    min_artifacts_per_run = config.get('min_artifacts_per_run', 1)
                    max_artifacts_per_run = config.get('max_artifacts_per_run', 1)
                    hash_chunk_size = config.get('hash_chunk_size', 1_048_576)
                        ...
                        for filename in filenames:
                            ...
                            try:
                                if self.artifact_exists(filepath) is False:
                                    ...
                                    self.register_artifact(filepath, hash_chunk_size)
                    ...
                    if self.exit_event.is_set() is True: 
                        ...
                        return
                    try:
                        self.resource_trigger(min_artifacts_per_run=min_artifacts_per_run)
    ...
    def register_artifact(self, filepath: str, hash_chunk_size: int) -> None:
        timestamp = datetime.now()

        with self.conn_manager.write_cursor() as cursor:
            cursor.execute(
                f"INSERT OR IGNORE INTO {self.artifact_table_name} (artifact_path, timestamp, artifact_hash, hash_algorithm) VALUES (?, ?, ?, ?);",
                (filepath, timestamp, self.hash_file(filepath, hash_chunk_size), "sha256")
            )
            cursor.execute(
                f"""
                ...
                VALUES (?, ?, ?, ?, ?, ?);
                """,
                (filepath, self.hash_file(filepath, hash_chunk_size), self.node_id, self.name, "detected", "input")
            )
    ...
    def __get_detected_artifacts(self, max_artifacts_per_run: int = None) -> list:
        with self.conn_manager.read_cursor() as cursor:
            cursor.execute(
                f"""
            ...
            if self.filtering_func is not None:
                ...
                for artifact_tuple in artifacts:
                    ...
                    if self.filtering_func(artifact_path) is True:
                        ...
                        if max_artifacts_per_run is not None:
                            if len(filtered_artifacts) < max_artifacts_per_run:
    ...
    def hash_file(self, filepath: str, hash_chunk_size: int) -> str:
    ...
    def resource_trigger(self, min_artifacts_per_run: int) -> None:
        if len(self.get_filtered_artifacts()) >= min_artifacts_per_run:
